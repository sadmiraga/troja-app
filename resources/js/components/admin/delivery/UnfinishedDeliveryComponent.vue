<template>
    ...
    <div class="drinks-food-create__container">
        <!-- DELTE MODAL  -->
        <div class="modal fade warning-popup" id="delete-delivery">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <h3 class="p-bold text-left">{{this.translations.Are_you_sure_you_want_to_delete_unfinsihed_delivery}} </h3>
                        <p  class="p-regular text-center mt-3">{{ this.translations.This_will_result_deleting_delivery_and_all_data_inside_of_it }}</p>
        ...
        <button type="button" id="delete-trigger-modal" class="btn btn-primary d-none" data-bs-toggle="modal" data-bs-target="#delete-delivery">
            Open Modal
        </button>
        <!-- SAVE MODAL -->
        <div class="modal fade warning-popup" id="save-delivery">
            <div class="modal-dialog">
                <div class="modal-content">
                    <!-- Modal Header close -->
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <h3 class="p-bold text-center">{{this.translations.Are_you_sure_you_want_to_save_unfinsihed_deivery}} </h3>
                        <p  class="p-regular text-center mt-3">{{this.translations.We_recommend_at_least_previewing_data_in_delivery_and_then_completing_it}}</p>
                    ...
                    <div class="modal-footer">
                        <div class="drinks-food-create-edit__bottom-buttons two-button-wrapper">
                            ...
                            <!-- CONFIRM -->
                            <a class="w-50" v-on:click="completeDelivery()" >
                                <button class="drinks-food-create-edit__add-allergens-button" style="border: 2px solid #1C6979;">
                                    {{ this.translations.Yes }}
                                </button>
                            </a>
        ...
        <button type="button" id="save-trigger-modal" class="btn btn-primary d-none" data-bs-toggle="modal" data-bs-target="#save-delivery">
            Open Modal
        </button>
        ...
        <div class="drinks-food-create-edit__form-container">
            ...
            <!-- DATE -->
            <p class="p-info">
                <span class="p-bold"> {{this.translations.Date}} </span>
                <span>{{ formatDateOnly(this.delivery.start) }}</span>
            </p>

            <!-- DATE -->
            <p class="p-info">
                <span class="p-bold">{{this.translations.Time}} </span>
                <span>{{ formatTimeOnly(this.delivery.start) }}</span>
            </p>

            <!-- LOCATION -->
            <p class="p-info">
                <span class="p-bold">{{this.translations.Location}} </span>
                <span>{{ this.location.name }}</span>
            </p>

            <!-- Storage -->
            <p class="p-info">
                <span class="p-bold">{{this.translations.Storage}} </span>
                <span>{{ this.delivery.storage }}</span>
            </p>
        ...
        <div class="drinks-food-create-edit__bottom-buttons two-button-wrapper">
            ...
            <!-- delete -->
            <button class="delete-button" v-on:click="deleteDeliveryTemp()">
        ...
        <div class="drinks-food-create-edit__bottom-buttons mt-3">
            <a class="w-100" :href="'/delivery/'+this.delivery.id+'/app'">
            <button>{{this.translations.Continue_delivery}}</button>
            </a>
        </div>
    </div>
...
<script>
    export default {
        ...
        props: ["translations","delivery","location"],

        methods:{

            deleteDeliveryTemp(){
                let button = document.getElementById('delete-trigger-modal');
                button.click();
            },

            saveDelivery(){
                let button = document.getElementById('save-trigger-modal');
                button.click();
            },
                ...
                return new Promise(resolve => {
                    window.axios.post(filter_url, {
                        ...
                        if(response.data == "success"){
                            this.$toast.success("You successfully completed / saved delivery.");
                            setTimeout(() => {
            ...
            deleteDelivery(){
                ...
                return new Promise(resolve => {
                    window.axios.post(filter_url, {
                        ...
                        if(response.data == "success"){
                            this.$toast.warning("You successfully deleted delivery.");
                            setTimeout(() => {
